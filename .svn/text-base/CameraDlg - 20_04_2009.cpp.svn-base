...
void CCameraDlg::FillCameraInfo()
{
	...
	CString serial1, serial2, serial3;
...
//////////////////////////////////
	...
	if(cam_id == CAM_01){  // CONTROL DE LA CABEZA
       ...
       m_strZ.Format(TEXT("%.3f"), varZ.dblVal);
		...
		for(i = 0; i < lCount; i++){         // por ahora sólo tenemos dos cosas para trackear
            CComPtr<INPObject> spObject;
            pFrame->Item(i, &spObject);           
            CComVariant varX2, varY2, varArea2;			
...
//		pmz = (int)p_mano01.z;
		...
		/////////////////// TRADUCCIÓN DE LAS COORDENADAS DEL MUNDO REAL A LAS COORDENADAS DE OGRE //////////////////
        if(p_mano01.x != 0.0f) pmz = -1 * ((((MAX_REAL_X - p_mano01.x) * (MAX_OGRE_REAL_X - MIN_OGRE_REAL_X)) / (MAX_REAL_X - MIN_REAL_X)) + MAX_OGRE_REAL_X);   //(MAX_OGRE_DIFF_X * (MAX_REAL_DIFF_X - p_mano01.x)) / (MAX_REAL_X)) - MIN_OGRE_REAL_X; 
        ...
        if(p_mano01.z != 0.0f) pmx = -1 * ((((MAX_REAL_Z - p_mano01.z) * (MAX_OGRE_REAL_Z - MIN_OGRE_REAL_Z)) / (MAX_REAL_Z - MIN_REAL_Z)) + MAX_OGRE_REAL_Z);   //((MAX_OGRE_DIFF_Z * (MAX_REAL_DIFF_Z - p_mano01.z)) / (MAX_REAL_Z)) - MIN_OGRE_REAL_Z; 
		...
		pmz -= 5;
		// adelante y atrás
		pmx += 5; // primero 10 después 7 y ahora 5

		// corrección de la altura
		float auxy2=0.0f;
		if(p_mano01.y <= 180){
			auxy2 = (((1.35f * (180 - p_mano01.y)) / 10) + 1.35f); 
		...
		//if((fabs(pmx - prev_pmx) >= 0.015f) && (fabs(pmx - prev_pmx) <= 0.035f)){ pmz = prev_pmz;  n_err_msg("", "zasca");}

       ////////// CONGELA EL EJE X SI HAY MOVIMIENTO EN Z

/*
		if((fabs(pmz - prev_pmz) >= 0.0015f)){ // && (fabs(pmx - prev_pmx) <= 0.035f)){ 
			 congelaX = true; 
			 pmx = prev_pmx;  
			 //n_err_msg("", "zasca");
             congela_contX += 127;
...
/*
        ///////////// CONGELA EL EJE Z SI HAY MOVIMIENTO EN X
		if((fabs(pmx - prev_pmx) >= 0.05f)){ // && (fabs(pmx - prev_pmx) <= 0.035f)){ 
			 congelaZ = true; 
			 pmz = prev_pmz;  
			 //n_err_msg("", "zasca");
             congela_contZ += 7;
...
*/
		...
		pauz = pmz;
        pmz = pmx;
		//pmz *= 1.22f; // 1.12 va guay
		
		pmz = pmz * (0.9f + (1/log(pmz)));
		if(pmz >= 23.0f && pmz < 23.25f) pmz *= 1.01;
		else if(pmz >= 23.25f && pmz < 23.5f ) pmz *= 1.02f;
		else if(pmz >= 23.5f && pmz < 23.75f)  pmz *= 1.04f;
		else if(pmz >= 23.75f && pmz < 24.0f)  pmz *= 1.07f;
		else if(pmz >= 24.0f && pmz < 24.25f)  pmz *= 1.10f;
		else if(pmz >= 24.25f && pmz < 24.5f)  pmz *= 1.14f;
		else if(pmz >= 24.5f && pmz < 24.75f)  pmz *= 1.19f;
		else if(pmz >= 24.75f)                 pmz *= 1.25f;
		
        if(pmz >= 24.3f) pmz = 24.3f;
        if(pmy >=  3.0f) pmy =  3.0f; // antes 1.5
        if(pmx >= 37.0f) pmx = 37.0f;
		...
		if(pmz >= 22 && pmz < 22.45){
		   if(pmx >= 40) pmx -= 0.01f;
		   else if(pmx < 40) pmx += 0.01f;
		}
		else if(pmz >= 22.45 && pmz < 23.25){
		   if(pmx >= 30) pmx -= 0.01f;
		   else if(pmx < 30) pmx += 0.01f;
		}
		else if(pmz >= 23.25 && pmz < 23.85){
		   if(pmx >= 20) pmx -= 0.01f;
		   else if(pmx < 20) pmx += 0.01f;
		}
		else if(pmz >= 23.85 && pmz < 24.2){
		   if(pmx >= 15) pmx -= 0.01f;
		   else if(pmx < 15) pmx += 0.01f;
		}
        //pmx = 30 - pmx;
...
//pmz -= 110;
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		int i2;
        LONG lCount2 = 0;
        pFrame->get_Count(&lCount2);

        // gestión de error - si hay más de dos puntos avisar
        //if(lCount > 4) n_err_msg("ADVERTENCIA", "Hay más de dos puntos visibles.\nSe usarán los 2 primeros encontrados (que quizás no sean información válida).");
        // SI HAY MÁS OBJETOS DE LOS QUE DEBERÍA, EN ESTE CASO - ME LA PELA
		for(i2 = 0; i2 < lCount2; i2++){         // por ahora sólo tenemos dos cosas para trackear
            CComPtr<INPObject> spObject2;
            pFrame->Item(i2, &spObject2);           
			...
			// este me obliga a crear un nuevo vec3 cada vez que sobreescribo un valor. malo para la memoria.
			//g_mano.addVertex(tVector3((float)(varX2.dblVal), (float)(varY2.dblVal), (float)(varArea2.dblVal)), i);         		
			g_mano2.addVertex((float)(varX22.dblVal), (float)(varY22.dblVal), (float)(varArea22.dblVal), i2);
			//n_err_msg("info", "gmano [%d]: %.2f %.2f %.2f", i, g_mano.vertices[i].x, g_mano.vertices[i].y, g_mano.vertices[i].z);
		}
		/////////////////////////////////////////////// REORDENAMIENTO DE LOS DEDOS ///////////////////////////////////
        int   k2=0,l2=0;
		...
		for(k2=0;k2<4;k2++){
			for(l2=0;l2<4;l2++){
				if(g_mano2.getVertexX(k2) > g_mano2.getVertexX(l2)){
					xaux2 = g_mano2.getVertexX(l2); // es una ELE no un UNO
					yaux2 = g_mano2.getVertexY(l2);
					zaux2 = g_mano2.getVertexZ(l2);

					g_mano2.vertices[l2].x = g_mano2.vertices[k2].x;
					g_mano2.vertices[l2].y = g_mano2.vertices[k2].y;
					g_mano2.vertices[l2].z = g_mano2.vertices[k2].z;

					g_mano2.vertices[k2].x = xaux2;
					g_mano2.vertices[k2].y = yaux2;
					g_mano2.vertices[k2].z = zaux2;
				}
			}
		}
		p_dedo21.x = g_mano2.vertices[0].x; p_dedo21.y = g_mano2.vertices[0].y; p_dedo21.z = g_mano2.vertices[0].z;
		p_dedo22.x = g_mano2.vertices[1].x; p_dedo22.y = g_mano2.vertices[1].y; p_dedo22.z = g_mano2.vertices[1].z;
		p_dedo23.x = g_mano2.vertices[2].x; p_dedo23.y = g_mano2.vertices[2].y; p_dedo23.z = g_mano2.vertices[2].z;
        p_mano21.x = g_mano2.vertices[3].x; p_mano21.y = g_mano2.vertices[3].y; p_mano21.z = g_mano2.vertices[3].z;

		//n_err_msg("err", "%f %f %f %f", p_dedo01.x, p_dedo02.x, p_dedo03.x, p_mano01.x);
        /////////////////////////////// AGREGADO PARA CALCULAR EL ÁNGULO DE LOS DEDOS/01/02/03  //////////////////////////////////
        float yaw_ang_dedo21=0.0f, yaw_ang_dedo22=0.0f, yaw_ang_dedo23=0.0f;
		float auxx2=0.0f, auxy2=0.0f, auxz2=0.0f;

        yaw_ang_dedo21 = atan(abs(p_dedo21.y - p_dedo22.y) / abs(p_dedo21.x - p_dedo22.x));
        if(p_dedo22.y < p_dedo21.y) yaw_ang_dedo21 *= -1.0f; 
		if(yaw_ang_dedo21 > 0.0f) yaw_ang_dedo21 = 0.0f; // AGREGADO PARA QUE LA ARTICULACIÓN NO VAYA MÁS ALLÁ DE LO QUE DEBERÍA

		yaw_ang_dedo22 = atan(abs(p_dedo22.y - p_dedo23.y) / abs(p_dedo22.x - p_dedo23.x));
        if(p_dedo23.y < p_dedo22.y) yaw_ang_dedo22 *= -1.0f; 
//        if(yaw_ang_dedo02 > 0.0f) yaw_ang_dedo02 = 0.0f; // AGREGADO PARA QUE LA ARTICULACIÓN NO VAYA MÁS ALLÁ DE LO QUE DEBERÍA

		yaw_ang_dedo23 = atan(abs(p_dedo23.y - p_mano21.y) / abs(p_dedo23.x - p_mano21.x));
//        if(p_mano01.y < p_dedo03.y) yaw_ang_dedo03 *= -1.0f; 

        matriz4[0][0] = (float)(cam_id);  // CAM_03 obviamente
		matriz4[0][1] = yaw_ang_dedo21;
	    matriz4[0][2] = yaw_ang_dedo22;
	    matriz4[0][3] = yaw_ang_dedo23;

	    ////////////////////////////////////// agregado para smooth de la mano ///////////////
		mano2x_smooth[curr_mano2_smooth] = (float)(p_mano21.x);
	    mano2y_smooth[curr_mano2_smooth] = (float)(p_mano21.y);
	    mano2z_smooth[curr_mano2_smooth] = (float)(p_mano21.z); // invierto el eje z para que el movimiento de la mano sea coherente con la realidad

        curr_mano2_smooth++;
	    if(curr_mano2_smooth > SMOOTH_MANO - 1) curr_mano2_smooth = 0;

		p_mano21.x = p_mano21.y = p_mano21.z = 0.0f;
	    for(i2=0;i2<SMOOTH_MANO;i2++){
	       p_mano21.x += mano2x_smooth[i2];
		   p_mano21.y += mano2y_smooth[i2];
	 	   p_mano21.z += mano2z_smooth[i2];
	    } 

		for(i2=0;i2<SMOOTH_MANO-(SMOOTH_MANO / 4);i2++){
	       auxx2 += mano2x_smooth[i2];
		   auxy2 += mano2y_smooth[i2];
	 	   auxz2 += mano2z_smooth[i2];
	    } 
        auxx2 /= (float)(SMOOTH_MANO - (SMOOTH_MANO / 4));
        auxy2 /= (float)(SMOOTH_MANO - (SMOOTH_MANO / 4));
        auxz2 /= (float)(SMOOTH_MANO - (SMOOTH_MANO / 4));
 
		if(auxx2 < 130)      auxx2 = 130.0f;
		else if(auxx2 > 310) auxx2 = 310.0f;

        if(auxz2 < 50)       auxz2 = 50.0f;
		else if(auxz2 > 120) auxz2 = 120.0f;

	    p_mano21.x /= (float)SMOOTH_MANO; //5.0f;
	    p_mano21.y /= (float)SMOOTH_MANO; //5.0f;
	    p_mano21.z /= (float)SMOOTH_MANO; //5.0f;

		mano2x_smooth[curr_mano2_smooth] = (float)(p_mano21.x);
	    mano2y_smooth[curr_mano2_smooth] = (float)(p_mano21.y);
	    mano2z_smooth[curr_mano2_smooth] = (float)(p_mano21.z);

		///////////////////////////////////// hasta acá /////////////////////////////////////
        float pmx2, pmy2, pmz2; // antes eran ints

		/////////////////// TRADUCCIÓN DE LAS COORDENADAS DEL MUNDO REAL A LAS COORDENADAS DE OGRE //////////////////
        if(p_mano21.x != 0.0f) pmz2 = -1 * ((((MAX_REAL_X - p_mano21.x) * (MAX_OGRE_REAL_X - MIN_OGRE_REAL_X)) / (MAX_REAL_X - MIN_REAL_X)) + MAX_OGRE_REAL_X);   //(MAX_OGRE_DIFF_X * (MAX_REAL_DIFF_X - p_mano01.x)) / (MAX_REAL_X)) - MIN_OGRE_REAL_X; 
        if(p_mano21.y != 0.0f) pmy2 = -1 * ((((MAX_REAL_Y - p_mano21.y) * (MAX_OGRE_REAL_Y - MIN_OGRE_REAL_Y)) / (MAX_REAL_Y - MIN_REAL_Y)) + MAX_OGRE_REAL_Y);   //((MAX_OGRE_DIFF_Y * (MAX_REAL_DIFF_Y - p_mano01.y)) / (MAX_REAL_Y)) - MIN_OGRE_REAL_Y; 
        if(p_mano21.z != 0.0f) pmx2 = -1 * ((((MAX_REAL_Z - p_mano21.z) * (MAX_OGRE_REAL_Z - MIN_OGRE_REAL_Z)) / (MAX_REAL_Z - MIN_REAL_Z)) + MAX_OGRE_REAL_Z);   //((MAX_OGRE_DIFF_Z * (MAX_REAL_DIFF_Z - p_mano01.z)) / (MAX_REAL_Z)) - MIN_OGRE_REAL_Z; 
  
		// normalización de valores
		pmx2 *= -0.5f; //-1;
		pmz2 *= -0.5f; //-1;
		pmy2 *= 0.65f; // antes 0.5

		// corrección de la profundidad (eje z mío, eje x rubén)
		pmz2 -= 5;
		// adelante y atrás
		pmx2 += 5; // primero 10 después 7 y ahora 5

		// corrección de la altura
		float auxy22=0.0f;
		if(p_mano21.y <= 180){
			auxy22 = (((1.35f * (180 - p_mano21.y)) / 10) + 1.35f); 
			pmy2 = auxy22 / 2.0f; //pmy + auxy2;
            pmy2 -= 0.7f;
			pmy2 -= 2.0f;
		}
		else{
		...
		pmz2 = pmz2 * (0.9f + (1/log(pmz2)));
		if(pmz2 >= 23.0f && pmz2 < 23.25f) pmz2 *= 1.01;
		else if(pmz2 >= 23.25f && pmz2 < 23.5f )  pmz2 *= 1.02f;
		else if(pmz2 >= 23.5f  && pmz2 < 23.75f)  pmz2 *= 1.04f;
		else if(pmz2 >= 23.75f && pmz2 < 24.0f)   pmz2 *= 1.07f;
		else if(pmz2 >= 24.0f  && pmz2 < 24.25f)  pmz2 *= 1.10f;
		else if(pmz2 >= 24.25f && pmz2 < 24.5f)   pmz2 *= 1.14f;
		else if(pmz2 >= 24.5f  && pmz2 < 24.75f)  pmz2 *= 1.19f;
		else if(pmz2 >= 24.75f)                   pmz2 *= 1.25f;
		
        if(pmz2 >= 24.3f) pmz2 = 24.3f;
        if(pmy2 >=  3.0f) pmy2 =  3.0f; // antes 1.5
        if(pmx2 >= 37.0f) pmx2 = 37.0f;
		//pmx = 22.4f; 

        pmx2 = pauz2;		
		if(pmz2 >= 22 && pmz2 < 22.45){
		   if(pmx2 >= 40)     pmx2 -= 0.01f;
		   else if(pmx2 < 40) pmx2 += 0.01f;
		}
		else if(pmz2 >= 22.45 && pmz2 < 23.25){
		   if(pmx2 >= 30)     pmx2 -= 0.01f;
		   else if(pmx2 < 30) pmx2 += 0.01f;
		}
		else if(pmz2 >= 23.25 && pmz2 < 23.85){
		   if(pmx2 >= 20)     pmx2 -= 0.01f;
		   else if(pmx2 < 20) pmx2 += 0.01f;
		}
		else if(pmz2 >= 23.85 && pmz2 < 24.2){
		   if(pmx2 >= 15)     pmx2 -= 0.01f;
		   else if(pmx2 < 15) pmx2 += 0.01f;
		}

		// MANO 01
		matriz4[0][4] = (float)(cam_id); 
	    matriz4[0][5] = (float)(pmx2); //p_mano01.x);
	    matriz4[0][6] = (float)(pmy2); //(p_mano01.y);
	    matriz4[0][7] = (float)(pmz2); //(p_mano01.z);
        
		if(first_time_cam02){
			if(abs(pmx2 - 240) < 7 && abs(pmx2 - 240) > 1){
			   if(abs(pmz2 - 90) < 7 && abs(pmz2 - 90) > 1){
				  first_time_cam02 = false;
			      n_err_msg("CAM03", "La mano está en su sitio.");			      
			   }
			}		
		}
        ...
        //m_strPitch.Format(TEXT("%.3f"), matriz3[0][2]);
        //m_strRoll.Format(TEXT("%.3f"), matriz3[0][3]);
        m_strYaw.Format(TEXT("%.3f"),   p_mano21.x);
        m_strPitch.Format(TEXT("%.3f"), p_mano21.y );
        m_strRoll.Format(TEXT("%.3f"),  p_mano21.z);
