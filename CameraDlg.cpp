...
float remix[40]; // variable usada para juntar la información de todas las cámaras, centralizarla, y enviarla
...
void CCameraDlg::UpdateVectorInfo(INPCameraFrame * pFrame)
	...
	if(cam_id == CAM_02){
		pFrame->get_Count(&lCount);
		// si no hay exactamente 2 puntos, se congelan los cálculos, ¿seguro que quiero hacerlo así y no coger los más a la izda?
		if(lCount >= 3){
